...
冒険者は `src/lib/names.ts` の名簿から引かれる（名60種 × 二つ名28種、一定確率で
「灰かぶりのユーリ」のように二つ名が付く）。初回起動時に登録画面が出るので、
名簿から引くか自分で書き入れるかを選べる。拠点パネルの「改名」でいつでも変更できる。

死亡すると名簿から新しい冒険者を迎え、代数が増える（拠点パネルの
「死亡したら名簿から新しい冒険者を迎える」でOFFにもできる）。
遠征記に記された名前は生成時に確定するので、過去の遠征記は当時の冒険者の名前を保つ。
記録庫が「歴代の遠征者の記録」になるのはこのため。
