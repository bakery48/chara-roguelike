...
2. 二つ名（特性）別の同じ指標と、特性なしとの比較
3. 強化コスト曲線
4. 放置したときの成長曲線（何時間で何階まで行けるか）と、実際に生成された遠征記
...
| | 決まり方 | 役割 |
|---|---|---|
| **二つ名**（`足早の` `薄氷の` …） | 抽選のみ。変えられない | **特性**。能力・確率・報酬を動かす |
| **名**（`ユーリ` `ハルカ` …） | 抽選 or 手入力。いつでも改名可 | 表示だけ。能力に影響しない |

こう分けているので、**自分で命名しても有利不利は生まれない**。初回起動時に登録画面が出て、
引いた二つ名とその効果を確認しながら、名だけを名簿から引くか自分で書き入れるかを選ぶ。
拠点パネルの「改名」でいつでも変えられるが、二つ名は本人のものなので動かない。

死亡すると二つ名ごと新しい冒険者を迎え、代数が増える（拠点パネルの
「死亡したら名簿から新しい冒険者を迎える」でOFFにもできる）。
遠征記の名前は生成時に確定するので、過去の遠征記は当時の冒険者の名前を保ち、
記録庫は「歴代の遠征者の記録」として読める。

### 二つ名の効果

効果の数値は `CONFIG.traits`（config.ts）、ラベルと由来は `names.ts` にある。
**画面に出る効果の説明文は config の数値から自動生成する**ので、文言と数値がずれることはない。
遠征に出た時点の特性は遠征データに焼き付けるため、途中で改名しても結果は変わらない。

`npm run balance` に二つ名別の表があり、安全な周回（目標5F）と深追い（目標10F・HP15%）の
両方で死亡率・平均到達階・遺産/分を基準（特性なし）と比べられる。現状は全26種が
**基準比 −1%〜+22%** に収まるよう調整してある。防御・死亡ロスト系は深追いでのみ効くので、
安全な設定だけで測ると死に特性に見える点に注意。
...
- 進行中の遠征は、100ms刻みで進めても8時間ぶんを一気に消化しても同じ結末になる
- したがってセーブ／ロードやオフライン復帰を跨いでも、遠征の顛末がぶれない
- 死亡時の後任も遠征の seed から引くので、同じ死からは同じ冒険者が出る

テストで検証している（`刻み幅を変えても結果が一致する` / `同じ遠征なら、刻み幅によらず同じ後任が選ばれる`）。

ただし**新しい遠征の seed は毎回引き直す**ので、「8時間放置した履歴」と「8時間見ていた履歴」が
一字一句同じになるわけではない。保証しているのは消化される進行量と、
個々の遠征が刻み幅に依存しないことの2点。
...
src/
  ...
  lib/
    ...
    names.ts             冒険者の名簿（名60種 / 二つ名＝特性26種）
